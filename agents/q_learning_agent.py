...
class QLearningAgent(BaseAgent):
    ...
    def q_table(self):
        ...
        return {state: self._fast_agent.q_table.table[state].tolist() for state in self._fast_agent.q_table.table}

    @q_table.setter
    def q_table(self, value: dict):
        # Expects a dict with lists and converts to ndarray for Cython
        converted_table = {state: np.array(q_values, dtype=np.float64) for state, q_values in value.items()}
    ...
    def load_q_table(self):
        ...
        if os.path.exists(self.q_table_file) and not os.getenv('PYTEST_CURRENT_TEST'):
            try:
                ...
                print(f"Warning: Could not read Q-table file '{self.q_table_file}'. Starting with an empty table.")
                self.q_table = {}
        else:
            self.q_table = {}
