...
def test_start_game_request_valid():
    request = StartGameRequest(human_player_symbol="X", player_x_type="Human", player_o_type="Random")
...
def test_start_game_request_invalid_symbol():
    with pytest.raises(ValidationError):
        StartGameRequest(human_player_symbol="Z", player_x_type="Human", player_o_type="Random")

def test_board_state_valid():
    board = [[" ", " ", " "], [" ", "X", " "], [" ", " ", " "]]
    state = BoardState(board=board, current_player="O", winner=None, winner_line=None, game_over=False)
...
def test_board_state_invalid_board_size():
    with pytest.raises(ValidationError):
        BoardState(board=[[" ", " "]], current_player="X", winner=None, winner_line=None, game_over=False)
