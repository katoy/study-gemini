...
def gui(mock_master):
    ...
    sys_modules = {"tkinter": MagicMock(), "tkinter.font": MagicMock()}

    # テストで使う実際のエージェントクラスをインポート
    ...
    with patch.dict("sys.modules", sys_modules):
        ...
        with patch("gui.SettingsUI") as MockSettingsUI, patch(
            "gui.BoardDrawer"
        ) as MockBoardDrawer, patch("gui.GameInfoUI") as MockGameInfoUI, patch(
            "gui.TicTacToe"
        ) as MockTicTacToe:
            ...
            # AGENT_CLASSESのモックを設定
            mock_settings_ui.AGENT_CLASSES = {
                "ランダム": RandomAgent,
...
def test_start_game_human_starts(gui):
    ...
    assert (
        not hasattr(gui, "agent_first_move_mock")
        or not gui.agent_first_move_mock.called
    )
...
def test_start_game_ai_starts(gui):
    ...
    # agent_first_move の内部をテストするためモックを解除 (または別のテストでカバー)
    # ここでは、agent_first_moveが呼ばれることと、その結果を検証する

    # agent_first_move内で必要となるモック
    agent_x_mock = MagicMock()
    agent_x_mock.get_move.return_value = (0, 0)  # AIが打つ手
    gui.mock_game.get_current_agent.return_value = agent_x_mock
    gui.mock_game.make_move.return_value = True  # AIの手が有効
    gui.mock_game.current_player = "X"  # AIのターン
    gui.mock_game.agent_player = "X"  # AIのプレイヤーシンボル

    gui.mock_board_drawer.draw_board.reset_mock()  # start_game内のdraw_board呼び出しをリセット
    ...
    # agent_first_move の内部が実行されたことの検証
    agent_x_mock.get_move.assert_called_once_with(gui.mock_game.board)
    assert gui.mock_board_drawer.draw_board.call_count == 2
...
def test_cell_clicked_human_wins(gui):
    """人間が手を打って勝利が確定するケースをテスト (カバレッジ向上用)"""
    gui.start_game()
    gui.mock_board_drawer.draw_board.reset_mock()  # start_game内のdraw_board呼び出しをリセット
    gui.mock_game.current_player = "X"
    gui.mock_game.human_player = "X"
    gui.mock_game.game_over = False
    gui.mock_game.make_move.return_value = True  # 有効な手
    gui.mock_game.check_winner.return_value = "X"  # Xが勝利する

    # game_overをモックして、実際にゲームオーバー処理が呼ばれるか確認
    gui.game_over = MagicMock()
    gui.cell_clicked(1, 1)

    gui.mock_game.make_move.assert_called_once_with(1, 1)
    gui.mock_board_drawer.draw_board.assert_called_once()
    gui.mock_game.check_winner.assert_called_once()
    gui.game_over.assert_called_once_with("X")
    gui.mock_game.switch_player.assert_not_called()  # 勝利した場合はプレイヤーは切り替わらない
...
@pytest.mark.parametrize(
    "agent_name, expected_class_name",
    [
        ("ランダム", "RandomAgent"),
        ("Minimax", "MinimaxAgent"),
        ("Database", "DatabaseAgent"),
