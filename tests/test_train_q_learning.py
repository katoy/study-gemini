...
class TestTrainQLearning(unittest.TestCase):
    ...
    def test_train_q_learning_agent_loop_and_reset(self, mock_tqdm, mock_agent_class, mock_game_class, mock_perfect_agent_class):
        ...
        type(mock_game_instance).game_over = PropertyMock(side_effect=[False, False, True] * num_episodes)
        ...
        mock_game_instance.get_current_agent.side_effect = [mock_agent, mock_opponent] * num_episodes
    ...
    @patch('train_q_learning.QLearningAgent')
    @patch('train_q_learning.tqdm')
    def test_train_q_learning_agent_winner(self, mock_tqdm, mock_agent_class, mock_game_class, mock_perfect_agent_class):
    ...
    @patch('train_q_learning.QLearningAgent')
    @patch('train_q_learning.tqdm')
    def test_train_q_learning_agent_no_move(self, mock_tqdm, mock_agent_class, mock_game_class, mock_perfect_agent_class):
    ...
    @patch('train_q_learning.QLearningAgent')
    @patch('train_q_learning.tqdm')
    def test_train_q_learning_agent_draw(self, mock_tqdm, mock_agent_class, mock_game_class, mock_perfect_agent_class):
        """引き分けの場合の報酬計算をテスト"""
        mock_agent = MagicMock()
        mock_agent.player = 'X'
        mock_agent.exploration_rate = 0.05
    ...
    @patch('train_q_learning.QLearningAgent')
    @patch('train_q_learning.tqdm')
    def test_train_q_learning_agent_loss(self, mock_tqdm, mock_agent_class, mock_game_class, mock_perfect_agent_class):
        """敗北の場合の報酬計算をテスト"""
        mock_agent = MagicMock()
        mock_agent.player = 'X'
        mock_agent.exploration_rate = 0.05
